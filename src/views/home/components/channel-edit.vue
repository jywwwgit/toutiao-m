...
export default {
  ...
  data () {
    return {
      ...
      localMyChannels: this.myChannels
    }
  },
  computed: {
    ...mapState(['user']),
  ...
  methods: {
    async loadAllChannels () {
      try {
        ...
        // console.log(this.localMyChannels)
        // if (!this.localMyChannels) {
    ...
    async onAddChannel (channel) {
      ...
      // 2.后台添加频道，需要数据[{id,seq:1},{id,seq:2},...],不包含推荐频道
      // 3.
      const orderChannels = this.myChannels.map((channel, i) => {
        return {
          id: channel.id,
          name: channel.name,
          seq: i
      ...
      if (this.user) {
        try {
          // 已登录，把数据请求接口放到线上
          await addUserChannel(orderChannels)
          this.localMyChannels.push({
            id: channel.id,
            name: channel.name
            // upLoading: false,
            // downLoading: false,
